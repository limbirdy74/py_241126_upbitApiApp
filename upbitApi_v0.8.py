...
class MainWindow(QMainWindow, form_class):  # slot 클래스
    def __init__(self):
        ...
        self.alarmFlag = 0  # 텔레그램 전송을 1회만 하기 위해. 구매, 판매시 1회만 처리하는 로직으로
    ...
    def printCoinData(self, coinPrice, signed_change_rate):  # slot 메소드
        ...
        # 텔레그램 전송
        if int(coinPrice) >= 134400000:
            self.telegram_message(f"지정 가격 도달1! 현재 {coinPrice}")
        if int(coinPrice) <= 134300000:
            if self.alarmFlag == 0:
                self.telegram_message(f"지정 가격 도달2! 현재 {coinPrice}")
                self.alarmFlag = 1

        print(f"비트코인의 이전가격 : {self.coinPrev}")
